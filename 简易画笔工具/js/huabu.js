let canvas;//定义全局画布
let context;//定义全局context
let img1 = [];//储存图像数组，用于撤销
let canX;//画布左上角的x坐标
let canY;//画布左上角y坐标
$(function () {
	...
	// img1.push(imgData);
	let paint = Object.create(Line);//定义父类，初始化获取画线条的对象
...
let Line = {
	name: "line",
	draw: function () {
		let painting = false;//初始化设置为不可画状态
		let p_x;//画笔初始x坐标
		let p_y;//画笔初始y坐标
		// let canvas = $('#cavs')[0];//获取画布的dom
		...
		$("#thickness").change(function (event) {
...
let eraser = {
	...
	draw: function () {
		...
		$("#thickness").change(function (event) {//修改粗细时，进行赋值
			context.lineWidth = $(this).val();
		});
	}
}
...
let download = {
	draw: function () {
		...
		// 下载后的文件名
		let filename = 'xiaoguangren' + (new Date()).getTime() + '.' + type;
		// download
